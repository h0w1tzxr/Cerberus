...
func renderGlobalHelp(out io.Writer) {
	...
	fmt.Fprintln(out, "  --token, -token      admin auth token")
	fmt.Fprintln(out, "  --tls-ca, -tls-ca    path to TLS CA certificate")
	fmt.Fprintln(out, "  --tls-server-name   TLS server name override")
	fmt.Fprintln(out, "")
